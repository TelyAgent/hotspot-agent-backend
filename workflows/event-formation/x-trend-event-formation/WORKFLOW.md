...
本阶段的证据边界是：热搜榜只能证明某个词、话题或说法在目标地区 X 热搜榜中出现、排名靠前、排名变化或跨地区同时出现；它不能单独证明现实事实为真。

事件创建或更新后，后续证据增强链路再根据 Event 的标题、normalized key、query 或关联热搜项追溯代表帖子、官方来源或外部证据。代表帖子抓取失败不得反向阻止 Event 形成，但必须在证据增强阶段如实记录。
